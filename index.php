...
SError::$LOG = true;
SError::$LOGTYPE = 'text';
SError::$LOGFILE="/tmp/grpc.error.log";


chdir(dirname(__FILE__));
define("ROOT_LIBS","libs/");
define("ROOT_PROTO_GENERATED","proto_generated/");
define("ROOT_SERVICES","services/");
spl_autoload_register(function($class){
	$root = ROOT_LIBS."/".str_replace("\\","/",$class).".php";
	if(is_file($root)){
		require_once($root);
	}
});
...
spl_autoload_register(function($class){
	$root = ROOT_SERVICES."/".str_replace("\\","/",$class).".php";
	if(is_file($root)){
		require_once($root);
	}
});
require_once("libs/vendor/autoload.php");
function getPostData($json_decode=false){
	$data=null;
...
$class = dirname($uri);
error_log(__FILE__.":".__LINE__.var_export($class,true)."\n",3,"/tmp/x.log");
...
if(class_exists($class)){
	...
	if(method_exists($a,$func)){
		...
		$r = Grpc\Parser::serializeMessage($r);
		echo $r;
	}else{
...
$ct = ob_get_contents();
error_log(__FILE__.":".__LINE__.var_export($ct,true)."\n",3,"/tmp/x.log");
